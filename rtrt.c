...
SphereIntersect (Ray * r, Color * p, float *dist, int level)
{
    ...
    b = 2.0 * (sphere.Q[0] * D[0] + sphere.Q[1] * D[1] + sphere.Q[2] * D[2]);
...
main (int argc, char *argv[])
{
    ...
    SDL_Window *window = SDL_CreateWindow ("Realtime Raytracing Demo",
                                           SDL_WINDOWPOS_CENTERED,
                                           SDL_WINDOWPOS_CENTERED, XSIZE,
                                           YSIZE, 0);
    if (!window) {
    ...
    while (running) {
        ...
        animate ();             // render to 'image' buffer

        // Update texture with new pixel data
        SDL_UpdateTexture (texture, NULL, image, XSIZE * 3);
